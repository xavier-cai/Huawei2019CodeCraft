...
void SchedulerDijkstra::DoInitialize(SimScenario& scenario)
{
    ...
    timeWeightForRoad.resize(roadCount, std::make_pair(Inf, Inf));
...
void SchedulerDijkstra::UpdateTimeWeight(SimCar* car)
{
    auto& carTrace = car->GetTrace();
    Cross* nextCross = car->GetCurrentCross();
    Road* currentRoad = car->GetCurrentRoad();
...
bool SchedulerDijkstra::UpdateCarTraceByDijkstraWithTimeWeight(const int& time, const SimScenario& scenario, const std::vector<int>& validFirstHop, SimCar* car) const
{
    //ASSERT(!car->GetIsInGarage());
    ASSERT(!car->GetIsReachedGoal());
    ASSERT(validFirstHop.size() > 0);

...
std::list<Car*>& SimRoad::GetCars(const int& lane) const
{
...
std::list<Car*>& SimRoad::GetCarsOpposite(const int& lane) const
{
    ASSERT(lane > 0 && lane <= m_road->GetLanes());
...
std::list<Car*>& SimRoad::GetCars(const int& lane, bool opposite) const
{
    return opposite ? GetCarsOpposite(lane) : GetCars(lane);
}

std::list<Car*>& SimRoad::GetCarsTo(const int& lane, const int& crossId) const
{
    return GetCars(lane, IsFromOrTo(crossId));
}

std::list<Car*>& SimRoad::GetCarsFrom(const int& lane, const int& crossId) const
{
    return GetCars(lane, !IsFromOrTo(crossId));
}
